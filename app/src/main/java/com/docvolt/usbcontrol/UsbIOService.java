...
public class UsbIOService extends Service{
    ...
    */
    ...
    public static void digitalWrite(int pin, int value) {
        if(((1 << pin) & mMode) == 0)
            return; // Ignore if pin is not in output mode

        // FTDI chips use active-low logic:
        // 0 in software turns ON the physical pin
        // 1 in software turns OFF the physical pin
        if(value == 0)
            mPins.set(mPins.get() | (1 << pin));  // Turn ON
        else
            mPins.set(mPins.get() & ~(1 << pin)); // Turn OFF
    }
